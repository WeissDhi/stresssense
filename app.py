...
def _alias_to_existing_column(model_columns_list, candidates):
    for candidate in candidates:
        if candidate in model_columns_list:
            return candidate
    return None
...
def preprocess_input(form):
    ...
    data = {
        ...
        "Degree": "BSc",
        "Have you ever had suicidal thoughts ?": form["suicidal_thoughts"],  # akan dialias jika nama berbeda
        "Work/Study Hours": 6,
        "Financial Stress": int(form["financial_stress"]),
        "Family History of Mental Illness": form["family_history"],  # akan dialias jika nama berbeda
        "Total_Pressure": int(form["academic_pressure"]) + int(form["work_pressure"]),
        # Sleep_Quality logic
    }

    # Alias dinamis: petakan nilai formulir Yes/No ke nama kolom aktual yang ada di model
    try:
        # Suicidal thoughts
        st_alias = _alias_to_existing_column(
            model_columns,
            [
        ...
        if st_alias:
            data[st_alias] = form.get("suicidal_thoughts", "No")

        # Family history
        fh_alias = _alias_to_existing_column(
            model_columns,
            [
        ...
        if fh_alias:
            data[fh_alias] = form.get("family_history", "No")

        # Financial problem
        fp_alias = _alias_to_existing_column(
            model_columns,
            [
                "Financial Problem",
                "Financial Problems",
        ...
        if fp_alias and "financial_problem" in form:
            data[fp_alias] = "Yes" if form.get("financial_problem") == "Yes" else "No"

        # Health issue(s)
        hi_alias = _alias_to_existing_column(
            model_columns,
            [
                "Health Issue",
                "Health Issues",
        ...
        if hi_alias and "health_issue" in form:
            data[hi_alias] = "Yes" if form.get("health_issue") == "Yes" else "No"
    except Exception as e:
        logger.warning("Dynamic alias mapping failed: %s", str(e))
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                fp = request.form.get("financial_problem", None)
                hi = request.form.get("health_issue", None)
                ...
                suicide_effect = 0.22 if st == "Yes" else 0.0
                family_effect = 0.12 if fh == "Yes" else 0.0
                financial_problem_effect = 0.12 if fp == "Yes" else 0.0
                health_issue_effect = 0.10 if hi == "Yes" else 0.0

                heuristic_risk = (
                    pressure_signal * 0.60
                    + diet_effect
                    + sleep_effect
                    + suicide_effect
                    + family_effect
                    + financial_problem_effect
                    + health_issue_effect
                )
                # clamp ke 0..1
                ...
                combined_prob = 0.82 * pred_prob + 0.18 * heuristic_risk
                combined_prob = _safe_clip(combined_prob)
                prediction = round(combined_prob * 100, 2)
                logger.debug(
                    "Pred raw=%.4f, heuristic=%.4f (st=%s, fh=%s, fp=%s, hi=%s), combined=%.4f",
                    pred_prob,
                    heuristic_risk,
                    st,
                    fh,
